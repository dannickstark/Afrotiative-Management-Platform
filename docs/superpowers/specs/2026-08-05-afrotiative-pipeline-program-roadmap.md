...
### SP6 — Auto-publication (contrôlée)
`autoPublishEnabled` (OFF défaut). Après scoring (SP4), si `score ≥ autoPublishMinScore` ET conditions de sûreté (aucun drapeau faible confiance, image présente, `sources ≥ autoPublishMinSources`) → auto-approuver + publier via `publishArticle`, avec **entrée `article_revisions`** d'audit (« publié automatiquement, score=… »). Mettre à jour README + spec SP5 pour acter la politique. Dépend de SP1 + SP4.
