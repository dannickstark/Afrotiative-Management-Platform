...
- **A `"use client"` component must never *value*-import a module reaching `@/db`** (`lib/studio/bindings.ts` does). `import type` is fine. Only `bun run build` catches a violation; run one when you change imports.
  - **Carve-out — *except through a file-level `"use server"` module* (added by the U0+U2 final review).** A `"use client"` file may value-import a `"use server"` module that itself reaches `@/db`, because the compiler cuts the graph at that boundary: the client side of the import is replaced by a `createServerReference` proxy (an id + a fetch stub), and nothing the server module imports is followed into the client bundle. `components/studio/editor-shell.tsx:30` is exactly this shape — `import { saveTemplateScene, publishTemplate } from "@/lib/actions/studio-actions"`, and `lib/actions/studio-actions.ts` is `"use server"` and does reach `@/db`. It is a **true-but-harmless** path, and it is the only one in the studio.
