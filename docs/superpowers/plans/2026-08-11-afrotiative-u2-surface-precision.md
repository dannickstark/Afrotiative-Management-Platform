...
- **A `"use client"` component must never *value*-import a module reaching `@/db`** (`lib/studio/bindings.ts` does). `import type` is fine. Only `bun run build` catches a violation; run one when you change imports.
  - **Carve-out — *except through a file-level `"use server"` module* (added by the U0+U2 final review).** A `"use client"` file may value-import a `"use server"` module that itself reaches `@/db`, because the compiler cuts the graph at that boundary: the client side of the import is replaced by a `createServerReference` proxy (an id + a fetch stub), and nothing the server module imports is followed into the client bundle. `components/studio/editor-shell.tsx:30` is exactly this shape — `import { saveTemplateScene, publishTemplate } from "@/lib/actions/studio-actions"`, and `lib/actions/studio-actions.ts` is `"use server"` and does reach `@/db`. It is a **true-but-harmless** path.
  - **How many such paths there are, measured rather than assumed.** This carve-out first said `editor-shell.tsx` was "the only one in the studio". That was wrong, and the error is the kind that matters: an auditor following the doc would expect a single flagged path and read the others as fresh violations. An import-graph walk over the 96 `"use client"` files finds **31 repo-wide**, of which **7 are in the studio** — `asset-library`, `editor-shell`, `manual-generate`, `preview-pane`, `render-mode`, `templates-table`, `version-history`. All of them are this same true-but-harmless shape.
  - **And the number that actually matters: real violations = 0.** No `"use client"` file value-imports a **plain** module reaching `@/db`. `lib/studio/bindings.ts` does reach `@/db`, but no client file value-imports it.
  - Evidence, not reasoning: the review inspected a **fresh browser bundle** for the studio page and found **0 real `@/db` paths**, plus **0 occurrences** of `DATABASE_URL`, `pg-pool`, `drizzle` and `new Pool`; the `createServerReference` proxies are present where the imports are, which is the positive proof that the boundary was cut rather than the module simply not being reached.
  - Practical consequence: a static "does any `"use client"` file transitively import `@/db`?" scan will **flag this and be wrong**. Any such scan must stop at a `"use server"` module, exactly as the compiler does. What the invariant still forbids, without exception, is a `"use client"` file value-importing a **plain** module (no `"use server"` header) that reaches `@/db` — `lib/diffusion/settings-core.ts` and `lib/studio/bindings.ts` are the two the repo has shipped wrong before.
