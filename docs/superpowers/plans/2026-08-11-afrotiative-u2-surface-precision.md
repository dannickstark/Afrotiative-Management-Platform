...
**Step 1 is a failing test, and it is the point of this task.** Establish the drift before changing any behaviour: for a layer at rotation 90°, dragging the `e` handle by a screen delta of `(0, +d)` should widen it by `d` (because +y on screen is the layer's local +x at 90°), and its centre should stay put. Assert against the current function and watch it fail. **If it does not fail, stop and report** — my analysis would then be wrong, and the rest of this task must not be built on it.
